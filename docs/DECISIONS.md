...
| --- | --- | --- | --- |
| D-043 | Accepted | `FINAL_EXCEL` là output chính, luôn là sheet đầu và chỉ có đúng 11 cột trong `FINAL_EXCEL_COLUMNS`; mọi audit/evidence/strategy sheet là debug phụ. Dữ liệu thiếu để trống và giải thích trong `GHI CHÚ`. | Project Owner đã chốt deliverable cuối theo 11 cột; contract `DATA`/`Trich xuat` và technical sheets trước đây phục vụ triển khai/debug, không phải output nghiệp vụ cuối. |
